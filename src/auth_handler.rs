use crate::auth::start_oauth_flow;
...
/// Set button title with bold white styling.
...
async fn run_auth_flow(
    ...
    if client_id.is_empty() {
        return Err("Client ID not set. Register an app at dev.kick.com first.".into());
    }
    if client_secret.is_empty() {
        return Err("Client Secret not set. Register an app at dev.kick.com first.".into());
    }

    // Kick uses OAuth2.1 with browser redirect — notify PI to open browser
    if let Some(inst) = openaction::get_instance(instance_id.to_string()).await {
        inst.send_to_property_inspector(json!({
            "event": "auth_started",
            "message": "Opening browser for Kick login..."
        }))
        .await?;
    }
